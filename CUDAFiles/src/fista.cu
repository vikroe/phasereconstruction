...
void Fista::allocate(){
    ...
    cudaMalloc(&c, 101*sizeof(double));
...
void Fista::iterate(double *input, int iters, bool warm, bool scaling){
    ...
    for(int iter = 0; iter < iters; iter++){
        ...
        if(!scaling){
            ...
            contractf_p<<<1,1>>>(1, &sumArr[N_BLOCKS], sumArr, &c[iter]);
        }

        //Cost calculation with sparsity constraint
        linear<<<N_BLOCKS,N_THREADS>>>(count, &c[iter], image, Imodel, temporaryf, false);
        ...
        scalef<<<1,1>>>(1, 2*t, &c[iter], &c[iter]);
        F2C<<<1,1>>>(1,&c[iter],newGuess);
    ...
    if(b_cost){
        ...
        if(!scaling){
            h_sumOfProducts(count, image, Imodel, sumArr);
            h_sumOfProducts(count, Imodel, Imodel, &sumArr[N_BLOCKS]);
            contractf_p<<<1,1>>>(1, &sumArr[N_BLOCKS], sumArr, &c[iters]);
        }

        //Cost calculation with sparsity constraint
        linear<<<N_BLOCKS,N_THREADS>>>(count, &c[iters], image, Imodel, temporaryf, false);
