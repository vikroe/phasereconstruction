...
AppData::AppData(const char* jsonFile){
    ...
    parseSettings(j);
    cudaMalloc(&d_modulus, z.size()*width*height*sizeof(uint8_t));
    cudaMalloc(&d_phase, z.size()*width*height*sizeof(uint8_t));
    cudaMallocHost(&inputFrame, width*height*sizeof(double));
    cudaMallocHost(&h_modulus, z.size()*width*height*sizeof(uint8_t));
    cudaMallocHost(&h_phase, z.size()*width*height*sizeof(uint8_t));
}
...
void AppData::parseSettings(json j){
    ...
    z =        j.at("z").get<std::vector<double>>();
